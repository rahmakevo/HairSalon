...
if(create.equals(remove)) {
	int mId = Integer.parseInt(id);
	stylist = new Stylist(username, userpassword);
	db.remove(mId);
}

List<Stylist> styles = db.selectAll();
%>

<form action="Stylist.jsp">
<table class="table">
<tr>
<td><input name="username" class="form-control" value="<%= stylist.getUsername() %>" placeholder="Select Stylist"></td>
<td><input name="password" class="form-control" value="<%= stylist.getUserpassword() %>" placeholder="Select Stylist"></td>
<td>
...
<%
for(Stylist style:styles) {
	%>
	<tr>
	<td><%= style.getUsername() %></td>
	<td><a href="Stylist.jsp?action=select&id=<%= style.getId() %>" class="btn btn-success">Select</a></td>
	<td><a href="Stylist.jsp?action=remove&id=<%= style.getId() %>" class="btn btn-danger">Delete</a></td>
	</tr>
	<%
}
%>
